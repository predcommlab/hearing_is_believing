'''
Quick script to run similarity encoders over all subjects.

All options from `subject_rsa_enc_foil.py` are available here.
'''
...
if __name__ == '__main__':
    ...
    n_workers = aux.get_opt('n_workers', default = 2, cast = int)               # number of parallel workers to use
    ...
    for subject in data.Subjects.trim():
        ...
        # run subject
        os.system(f"python subject_rsa_enc_foil.py id={subject.sid} fs={fs} n_k1={n_k1} n_topk={n_topk} pad_L={pad_L} b_brain={int(b_brain)} b_clear={int(b_clear)} b_morph={int(b_morph)} n_mod={n_mod} n_folds={n_folds} n_permutations={n_permutations} n_workers={n_workers} n_alphas={n_alphas} b_acoustic={int(b_acoustic)} b_semantic={int(b_semantic)} backend={backend} device={device}")