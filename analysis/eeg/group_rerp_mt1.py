...
if __name__ == '__main__':
    ...
    for subject in data.Subjects.trim():
        ...
        # run subject
        os.system(f"python subject_rerp_mt1.py id={subject.sid} fs={fs} b_zsc={int(b_zsc)} b_spa={int(b_spa)} s_mod={s_mod} s_bsl={s_bsl} n_features={n_features} b_bsl={int(b_bsl)} n_workers={n_workers} n_alphas={n_alphas} n_folds={n_folds} n_permutations={n_permutations} n_topk={n_topk} n_edge_l={n_edge_L} n_maxl={n_maxL} n_tmin={n_tmin} n_tmax={n_tmax} backend={backend} device={device} maxtask={maxtask}")