'''
Quick script to run knock-out analyses over data from exemplar task.

All options available in `subject_rerp_mt2.py` are, of course, also
available here.
...
if __name__ == '__main__':
    ...
    for subject in data.Subjects.trim():
        ...
        # run subject
        os.system(f"python subject_rerp_mt2_knockout.py id={subject.sid} fs={fs} b_con={int(b_con)} b_inc={int(b_inc)} b_zsc={int(b_zsc)} b_spa={int(b_spa)} b_bsl={int(b_bsl)} s_mod={s_mod} n_mod={n_mod} a_coefs={a_coefs} n_workers={n_workers} n_alphas={n_alphas} n_folds={n_folds} n_permutations={n_permutations} n_topk={n_topk} n_edge_l={n_edge_L} n_maxl={n_maxL} n_tmin={n_tmin} n_tmax={n_tmax} backend={backend} device={device} maxtask={maxtask}")