'''
Quick script to collect data from rERP knock-outs in the final task
...
if __name__ == '__main__':
    ...
    for s_i, subject in enumerate(subjects):
        ...
        f = f'{dir_out}rerp-mt2-ko-n{n_mod}-c{a_coefs}-c{int(b_con)}-i{int(b_inc)}-k{n_topk}-z{int(b_zsc)}-s{int(b_spa)}-b{int(b_bsl)}-{s_mod}.pkl.gz'
        ...
        # load data for delta
        f = f'{dir_out}rerp-mt2-c{int(b_con)}-i{int(b_inc)}-k{n_topk}-z{int(b_zsc)}-s{int(b_spa)}-b{int(b_bsl)}-{s_mod}.pkl.gz'
    ...
    if s_mod == 'spc':
        if (b_con == 1) & (b_inc == 0):
            r = r[:,[0, 1, 2, 3],:]
        elif (b_con == 0) & (b_inc == 1):
            r = r[:,[0, 4, 5, 6],:]
    
    '''
    Correct order of coefficients
    
    NOTE: This is necessary because, for technical reasons,
    the order of the lagged coefficients is flipped in the
    knock-out procedure. Technically, I had meant to correct
    for that post knock-out, but for the MT2 file I forgot.
    
    Hence, we now need to do it here. Be aware that I did
    correctly handle this for MT1 files.
    '''
    ...
    report = mne.Report(title = f'rERP-ko: mt2 c{int(b_con)} i{int(b_inc)} {s_mod} m{n_mod}', verbose = False)
    ...
    for coef in range(R.shape[2]):
        ...
        with warnings.catch_warnings():
            ...
            t, c, p, _ = mne.stats.spatio_temporal_cluster_1samp_test(R2_c.swapaxes(1, 2), 
                                                                      ...
                                                                      tail = 0.0, threshold = n_cbt, adjacency = adj1, verbose = False)
            mask = np.zeros((len(p),))
    ...
    f = f'{dir_out}rerp-mt2-ko-m{n_mod}-c{int(b_con)}-i{int(b_inc)}-k{n_topk}-z{int(b_zsc)}-s{int(b_spa)}-b{int(b_bsl)}-{s_mod}.pkl.gz'
    ...
    f = f'{dir_out}rerp-mt2-ko-m{n_mod}-c{int(b_con)}-i{int(b_inc)}-k{n_topk}-z{int(b_zsc)}-s{int(b_spa)}-b{int(b_bsl)}-{s_mod}.html'
