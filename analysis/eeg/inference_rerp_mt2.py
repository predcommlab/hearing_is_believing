'''
Quick script to collect data from rERP models in final task
and perform statistical inference.

All options supported by `subject_rerp_mt2.py` are
...
if __name__ == '__main__':
    ...
    report = mne.Report(title = f'rERP: mt2 c{int(b_con)} i{int(b_inc)} {s_mod}', verbose = False)
    ...
    f = f'{dir_out}rerp-mt2-c{int(b_con)}-i{int(b_inc)}-k{n_topk}-z{int(b_zsc)}-s{int(b_spa)}-b{int(b_bsl)}-{s_mod}.html'
