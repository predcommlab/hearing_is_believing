'''
This script performs knock-out analyses for rERP models in the exemplar task.

Briefly, because we use dense design matrices, raw coefficients lose some of
the interpretability (i.e., we cannot interpret time courses as impulse responses
to sparse events, but as responses relative to a baseline instead). To alleviate
this issue, here we refit the relevant models to the data and then perform a knock-
out procedure over coefficients of interest (i.e., remove one coefficient at one 
lag at a time and reevaulate predictive performance) to get a better estimate of
the spatiotemporal signature of our effects of interest.

NOTE: Should you want to run this script on your CPU, please consider
still running it with `backend=torch` nevertheless. This is because the
torch implementation is much faster than numpy for predictions. For
reference, here is a timing test:

    %timeit trf_numpy.estimator.predict(X_n)
    ` 63.4 ms ± 1.54 ms per loop (mean ± std. dev. of 7 runs, 10 loops each)
    %timeit trf_torch.estimator.predict(X_t)
    ` 8.29 ms ± 196 µs per loop (mean ± std. dev. of 7 runs, 100 loops each)

NOTE: Here, we accept all arguments that `subject_rerp_mt2.py` accepts. However,
we also expect  `n_mod` to indicate which model should be evaluated using the 
knock-out procedure. This should always be the winning model per configuration,
so make sure to check results from `group_rerp_mt1.py` before running this. Similarly,
please supply `a_coefs` (e.g., '-4,-3,-2,-1' or simply `all`).
...
OPTIONS
    ...
    n_mod               -   Which model to evaluate (default = 2)
    a_coefs             -   Which coefficients to evaluate (default = all)
...
EXAMPLE USAGE:
    python subject_rerp_mt2_knockout.py id=0002 n_permutations=10 backend=torch device=cuda n_mod=2 a_coefs=all
'''
...
def permutation_worker(task: Dict, settings: Union[tuple, None] = None) -> Union[int, Dict]:
    ...
    INPUTS:
        ...
        settings    -   Settings. Tuple includes:
                            ...
                            n_mod:      Which model should we refit?
                            a_coefs:    Which coefficients should be evaluated?
    ...
    OUTPUTS:
        result      -   Results dictionary for permutation. Dictionary includes:
                            p_i:    Permutation index
                            ß:      Beta estimates (`channels` x `features` x `time`)
                            ß_r2:   Beta estimates, but R2 transformed (`channels` x `features` x `time`)
                            R:      Knock-out contribution of each specified coefficient (`channels` x `features` x `time`)
                            r:      Out of sample accuracy (pearson correlation) (`channels`)
    '''
    
    # unpack task data and settings
    p_i, indc, i = task['p_i'], task['indc'], task['i']
    ts, N, n_tmin, n_tmax, fs, n_folds, alphas, b_spa, X, X_mask, fit_models, y, n_mod, a_coefs, backend, device = settings
    
    # log progress
    aux.progressbar(i, N, ts, msg = f'[ERP]')
    
    # check coefs
    if isinstance(a_coefs, str): a_coefs = np.arange(X_mask[n_mod].sum())
    m_coefs = np.where(X_mask[n_mod])[0][a_coefs]
    ...
    if backend == 'torch':
        ...
        a_coefs, m_coefs = torch.from_numpy(a_coefs).to(torch.int64).to(device), torch.from_numpy(m_coefs).to(torch.int64).to(device)
    ...
    if backend == 'torch':
        ß = torch.zeros((n_folds, y.shape[1], X.shape[1], trf.window.shape[0]), dtype = X.dtype, device = X.device)
        R = torch.zeros((n_folds, y.shape[1], X.shape[1], trf.window.shape[0]), dtype = X.dtype, device = X.device)
        r = torch.zeros((n_folds, y.shape[1]), dtype = X.dtype, device = X.device)
    else:
        ß = np.zeros((n_folds, y.shape[1], X.shape[1], trf.window.shape[0]))
        R = np.zeros((n_folds, y.shape[1], X.shape[1], trf.window.shape[0]))
        r = np.zeros((n_folds, y.shape[1]))
    ...
    for m_i, mask in enumerate(X_mask):
        # here, we fit only the model of interest
        if m_i != n_mod: continue
        
        # check if model is required (principally irrelevant)
        ...
        for f_i, (train, test) in enumerate(kf.split(X, y)):
            ...
            if backend == 'torch': 
                ß[f_i,:,mask,:] = trf.coef_.to(ß.dtype)
                r[f_i,:] = trf.score(X_i[test,:,:], y[test,:,:]).to(r.dtype)
            else:
                ß[f_i,:,mask,:] = trf.coef_.swapaxes(0, 1)
                r[f_i,:] = trf.score(X_i[test,:,:], y[test,:,:])
            
            # create dummy
            X_d, y_d = trf.make_design(X_i[test,:,:], y[test,:,:])
            
            # scale
            X_d, y_d = trf.scaler_X.transform(X_d), trf.scaler_y.transform(y_d)
            
            # clone
            if backend == 'torch': X_d_i = X_d.clone()
            else: X_d_i = X_d.copy()
            
            # evaluate knock-outs
            for c_i, (c, c_m) in enumerate(zip(a_coefs, m_coefs)):
                # correct c
                if c < 0: c = X_i.shape[1] + c

                # loop over lags
                for l_i in range(trf.window.shape[0]):
                    '''
                    NOTE: Becaue of the way the design matrix is constructed,
                    it is important to know that, technically, the order of
                    coefficient lags here is flipped, i.e., the first lag at 
                    l_i = 0 will correspond to the real last lag l_t = T.
                    
                    The `TimeDelayed` class typically correct for this before
                    returning coefficients, but given that we are working with
                    its estimator class here, this correction is not applied.
                    
                    This is completely irrelevant for this particular script,
                    but the resulting trajectories must be flipped at some
                    point before inference. Here, I must admit that I forgot
                    to apply this correction, but we are doing it before
                    running stats in `inference_rerp_mt2_knockout.py`.
                    '''
                    
                    # knock-out coef at lag
                    X_d_i[:,c+l_i*X_i.shape[1]] = 0.0
                    
                    # obtain knock-out predictions
                    y_h_i = trf.estimator.predict(X_d_i)
                    
                    # score predictions
                    if backend == 'torch': R[f_i,:,c_m,l_i] = (r[f_i] - rsa.math.torch.pearsonr(y_h_i.T, y_d.T)).to(R.dtype)
                    else: R[f_i,:,c_m,l_i] = r[f_i] - rsa.math.pearsonr(y_h_i.T, y_d.T)
                    
                    # reset coef at lag
                    X_d_i[:,c+l_i*X_i.shape[1]] = X_d[:,c+l_i*X_i.shape[1]]
    ...
    with warnings.catch_warnings():
        ...
        ß_r2 = ß / np.abs(ß).sum(axis = 2, keepdims = True).max(axis = 3, keepdims = True)
        
        # average over folds
        ß = np.nanmean(ß, axis = 0)
        R = np.nanmean(R, axis = 0)
        ß_r2 = np.nanmean(ß_r2, axis = 0)
        r = np.nanmean(r, axis = 0)
    
    return dict(p_i = p_i, ß = ß, ß_r2 = ß_r2, R = R, r = r)
...
if __name__ == '__main__':
    ...
    n_mod = aux.get_opt('n_mod', default = 2, cast = int)                       # which model do we test here? (default = 2)
    a_coefs = aux.get_opt('a_coefs', default = 'all', cast = str)               # which coefficients to test? (default = all)
    ...
    ts = time.time()
    outputs = processor.run(jobs, external = permutation_worker, settings = (ts, len(jobs), n_tmin, n_tmax, fs, n_folds, alphas, b_spa, X, X_mask, fit_models, y, n_mod, a_coefs, backend, device))
    
    # allocate memory
    n_delays = int((n_tmax - n_tmin) * fs) + 1
    
    ß = np.zeros((n_permutations, y.shape[1], X.shape[1], n_delays))
    ß_r2 = np.zeros_like(ß)
    R = np.zeros((n_permutations, y.shape[1], X.shape[1], n_delays))
    ...
    with warnings.catch_warnings():
        ...
        with gzip.open(dir_out + f'rerp-mt2-ko-n{n_mod}-c{a_coefs}-c{int(b_con)}-i{int(b_inc)}-k{n_topk}-z{int(b_zsc)}-s{int(b_spa)}-b{int(b_bsl)}-{s_mod}.pkl.gz', 'wb') as f:
            pickle.dump((np.nanmean(R, axis = 0), np.nanmean(ß, axis = 0), np.nanmean(ß_r2, axis = 0), np.nanmean(r, axis = 0)), f)